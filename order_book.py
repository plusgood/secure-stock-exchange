	...
	def match(self):
		...
		if best_bid.price < best_ask.price:
			...
			if best_bid.quantity == 0:
				...
				self.history.append("Bid {0} filled, removed from orderbook".format(best_bid.id))
			if best_ask.quantity == 0:
				self.asks.remove(best_ask)
				self.history.append("Ask {0} filled, removed from orderbook".format(best_ask.id))
			self.match()
...
class Order:
	def __init__(self, price, quantity, direction, id):
		self.price = price
